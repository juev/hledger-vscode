...
Реализация согласована 2026-09-13. После ручной проверки пользователь уточнил
цикл: новый вызов открывает короткий список, повторный Ctrl+Space расширяет
его, Escape закрывает и сбрасывает полный режим.
...
3. **Продолжить ввод:** полный режим сохраняется при уточнении имени, включая
   `:` и Backspace в открытом списке. Escape закрывает список и сбрасывает
   режим. Ctrl+Space с закрытым списком начинает с короткого, независимо от
   предыдущего режима. Принятие подсказки, переход к сумме, выход курсора за
   имя счёта или смена редактора также сбрасывают режим.
...
Для отслеживания ввода режим привязан к текущему имени счёта. Его явный сброс
выполняют две команды extension: Escape при открытом account-списке и
Ctrl+Space при закрытом списке. Условия bindings используют
`suggestWidgetVisible`; считать provider requests для этого не требуется.
Программное скрытие списка внутри расширения сохраняет подготовленный ответ.
Смена окна сама по себе режим не сбрасывает; смена редактора сбрасывает.
...
- Extension, Node 22.23.1: `npm run typecheck`, `npm run typecheck:test`,
  ...
  После изменения цикла проходят 809 тестов; coverage thresholds из CI соблюдены.
- VS Code 1.137.0, отдельный профиль, собранный LSP и настоящий LanguageClient:
  ...
  Расширение не изменило текст. В исходной реализации Escape/reopen и
  Backspace сохраняли полный режим; после ручной проверки поведение Escape
  изменено. Принятие через Tab и мышью вернуло следующий запрос к `nonzero`.
...
- После добавления сброса по Escape пользователь вручную проверил локальную
  сборку и подтвердил цикл «короткий → полный → Escape → короткий».
...
- Round 4: после ручной проверки пользователь запросил новый цикл после
  Escape. Добавлен сброс по Escape и Ctrl+Space при закрытом списке, включая
  отмену ожидающего полного ответа. Ввод и Backspace сохраняют режим.
