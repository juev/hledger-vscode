...
export class HLedgerEnterKeyProvider {
    ...
    static handleEnterKey(
        ...
        if (currentLineText.match(/^(\d{4}[-/.]?\d{1,2}[-/.]?\d{1,2}|\d{1,2}[-/.]?\d{1,2})\s*(\*|!)?\s*(\([^)]+\))?\s*\S.*$/)) {
            return {
        ...
        if (currentLineText.match(/^\s+\S.*$/)) {
            ...
            const currentIndent = (indentMatch?.[1]) ? indentMatch[1] : '    ';
...
export class HLedgerEnterCommand implements vscode.Disposable {
    ...
    private async onEnter(textEditor: vscode.TextEditor, edit: vscode.TextEditorEdit): Promise<void> {
        ...
        const edits: { selection: vscode.Selection; action: ReturnType<typeof HLedgerEnterKeyProvider.handleEnterKey> }[] = [];
