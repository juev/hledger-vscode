...
export class HLedgerTabCommand implements vscode.Disposable {
    ...
    private async applySmartPositioning(
        ...
        try {
            ...
            if (alignmentPosition !== null) {
                ...
                // Show brief indication of smart positioning
                vscode.window.setStatusBarMessage('hledger: Aligned to amount position', 1500);
            } else {
    ...
    private async getOptimalAmountPosition(
        ...
        try {
            ...
            // Find the transaction that contains the current line
            const currentTransaction = this.findTransactionForLine(transactions, currentLineNumber + 1);
     ...
     */
    private findTransactionForLine(transactions: TransactionBlock[], lineNumber: number): TransactionBlock | null {
        for (const transaction of transactions) {
            // Check if the line is within the transaction range
            if (lineNumber === transaction.headerLineNumber ||
