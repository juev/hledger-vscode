    ...
    vscode.Disposable
{
  ...
  provideDocumentSemanticTokens(
    ...
    if (this.tokenCache.size > 20) {
      const entriesToRemove = Math.max(
        1,
        Math.floor(this.tokenCache.size * 0.25),
      );
  ...
  private buildTokens(
    ...
    for (
      let line = startLine;
      line <= endLine && line < document.lineCount;
      line++
    ) {
      ...
      if (header) {
        ...
        // status operator
        if (status)
          this.pushGroup(builder, line, text, base, full, status, "operator");
        // code (parentheses)
        ...
        // note (starts with '|')
        if (note) {
          const noteText = note.startsWith("|")
            ? note.slice(1).trimStart()
            : note;
      ...
      const dirMatch =
        /^(account|alias|commodity|decimal-mark|include|P|D|Y|payee|tag)\b/.exec(
          text,
        );
      if (dirMatch && typeof dirMatch[1] === "string") {
        const kw = dirMatch[1];
        if (kw.length > 0)
          this.push(builder, line, dirMatch.index, kw.length, "directive");
      }
      ...
      if (posting) {
        ...
        const remainderRel =
          remainder.length > 0 ? whole.indexOf(remainder) : -1;
        const remainderStart =
          remainderRel >= 0 ? baseIndex + remainderRel : -1;
      ...
      if (pipeIdx >= 0 && (inlineIdx < 0 || pipeIdx < inlineIdx)) {
        ...
        const noteStart =
          pipeIdx + 1 + (text.length - (pipeIdx + 1) - noteText.length);
