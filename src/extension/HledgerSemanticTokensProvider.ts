...
export class HledgerSemanticTokensProvider implements vscode.DocumentSemanticTokensProvider, vscode.DocumentRangeSemanticTokensProvider {
  ...
  private buildTokens(document: vscode.TextDocument, range?: vscode.Range): vscode.SemanticTokens {
    ...
    const amountSuffixRe = /([-+]?(?:\d{1,3}(?:[\s,']\d{3})*|\d+)(?:[.,]\d+)?)(?:\s+([\p{L}\p{N}\p{Sc}]+|"[^"]+"))?/gu;
    ...
    for (let line = startLine; line <= endLine && line < document.lineCount; line++) {
      ...
      const dirMatch = /^(account|alias|commodity|decimal-mark|include|P|D|Y|payee|tag)\b/.exec(text);
      ...
      if (posting) {
        ...
        if (remainderStart >= 0) {
          this.pushAllMatchesWithOffset(builder, line, remainder, remainderStart, opBalanceAssertRe, 'operator');
          this.pushAllMatchesWithOffset(builder, line, remainder, remainderStart, opPriceAssignRe, 'operator');
          this.pushAllMatchesWithOffset(builder, line, remainder, remainderStart, opCostTotalRe, 'operator');
          this.pushAllMatchesWithOffset(builder, line, remainder, remainderStart, opCostUnitRe, 'operator');

          // Amounts and commodities (both forms)
          this.pushAmounts(builder, line, remainder, remainderStart, amountSuffixRe, amountPrefixRe);
        }
      }

      // 6) Periodic and timeclock operators at line start
      const periodic = opPeriodicRe.exec(text);
      if (periodic && typeof periodic[1] === 'string') this.push(builder, line, periodic.index, periodic[1].length, 'operator');
  ...
  private push(builder: vscode.SemanticTokensBuilder, line: number, startChar: number, length: number, type: TokenType): void {
  ...
  private pushAmounts(
    ...
    while ((m = suffixRe.exec(text))) {
      ...
      if (full.length === 0) { suffixRe.lastIndex++; continue; }
    ...
    while ((m = prefixRe.exec(text))) {
      ...
      if (full.length === 0) { prefixRe.lastIndex++; continue; }
      if (typeof commodity === 'string' && commodity.length > 0) {
        const commodityStart = full.indexOf(commodity);
        if (commodityStart >= 0) this.push(builder, line, offset + m.index + commodityStart, commodity.length, 'commodity');
      }
