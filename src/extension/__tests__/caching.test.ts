...
`;
      ...
      expect(payees).toEqual(
        expect.arrayContaining(['Магазин Пятёрочка', 'ТЦ Европейский', 'Заправка Лукойл'])
      );
    });
...
`;
      ...
      expect(tags).toEqual(expect.arrayContaining(['category', 'type', 'cuisine']));
    });
...
`;
      ...
      expect(config.getPayees()).toEqual(
        expect.arrayContaining(['Магазин Пятёрочка', 'ТЦ Европейский'])
      );
      expect(config.getTags()).toEqual(expect.arrayContaining(['category', 'type']));
    });
...
`;
      ...
      const tags = config.getTags();
      expect(tags).toEqual(expect.arrayContaining(['category', 'subcategory', 'type', 'priority']));
    });
  });
});
