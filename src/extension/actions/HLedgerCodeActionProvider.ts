...
export class HLedgerCodeActionProvider implements vscode.CodeActionProvider {
  ...
  private createBalanceAssertionAction(
    ...
    const action = new vscode.CodeAction('Add balance assertion', vscode.CodeActionKind.Refactor);

    const edit = new vscode.WorkspaceEdit();
    const assertionText = this.buildAssertionText(postingInfo.amount, postingInfo.commodity);

    const insertPosition = new vscode.Position(range.start.line, lineText.length);
  ...
  private parsePostingLine(lineText: string): PostingInfo | null {
    ...
    const postingPattern =
      /^\s{2,}([\p{L}\p{N}:_\s-]+?)\s{2,}([\p{Sc}])?(-?[\p{N}][\p{N},.]*)\s*(\p{Sc}|[A-Z]{3,})?(\s*=\s*)?/u;
  ...
  private createFixTagFormatQuickFix(
    ...
    action.edit.replace(document.uri, diagnostic.range, `${tagName}:`);
