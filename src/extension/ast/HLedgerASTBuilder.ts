...
export class HLedgerASTBuilder {
   ...
   */
  private handleTransactionToken(
    token: HLedgerToken,
    data: MutableParsedHLedgerData,
    context: BuildingContext
  ): void {
    ...
    const dateMatch = token.trimmedLine.match(
  ...
  private pruneTemplates(templates: Map<TemplateKey, MutableTransactionTemplate>): void {
    const entries = Array.from(templates.entries()).sort(
      (a, b) => a[1].usageCount - b[1].usageCount
    );
