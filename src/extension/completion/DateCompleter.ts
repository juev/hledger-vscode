...
export class DateCompleter {
  ...
  complete(context: CompletionContext): vscode.CompletionItem[] {
    ...
    completions.push(
      this.createDateItem(yesterday, DateCompletionType.Yesterday, useShortFormat, context)
    );
    ...
    completions.push(
      this.createDateItem(lastWeek, DateCompletionType.LastWeek, useShortFormat, context)
    );

    // Start of this month
    const monthStart = new Date(today.getFullYear(), today.getMonth(), 1);
    completions.push(
      this.createDateItem(monthStart, DateCompletionType.MonthStart, useShortFormat, context)
    );
    ...
    completions.push(
      this.createDateItem(
        lastMonthStart,
        DateCompletionType.LastMonthStart,
        useShortFormat,
        context
      )
    );
  ...
  ): ValidationResult<CompletionContext> {
    const errors: string[] = [];
  ...
  private createDateItem(
    date: Date,
    description: DateCompletionType,
  ...
  private getSortPrefix(description: DateCompletionType): string {
    switch (description) {
      case DateCompletionType.Today:
        return '0001';
      case DateCompletionType.Yesterday:
        return '0002';
      case DateCompletionType.LastWeek:
        return '0003';
      case DateCompletionType.MonthStart:
        return '0004';
      case DateCompletionType.LastMonthStart:
        return '0005';
      case DateCompletionType.LastUsed:
        return '0000';
      default: {
