...
export class TagCompleter {
  ...
  private completeTagValues(context: CompletionContext, tagName: TagName): vscode.CompletionItem[] {
    ...
    if (context.document) {
      ...
      const vsDocument = vscode.workspace.textDocuments.find(
        (doc) => doc.uri.path === context.document!.uri
      );
      if (vsDocument) {
    ...
    const items = matches.map((match) =>
      this.createTagValueCompletionItem(match, tagName, context)
    );
   ...
   */
  private createTagValueCompletionItem(
    match: FuzzyMatch,
    tagName: TagName,
   ...
   */
  private createTagNameCompletionItem(
    match: FuzzyMatch,
    context: CompletionContext
  ): vscode.CompletionItem {
    ...
    if (usageCount > 1) {
      item.documentation = new vscode.MarkdownString(`Tag \`${tagName}\` used ${usageCount} times`);
    }
    ...
    if (tagValues.length > 0) {
      const existingDoc =
        (item.documentation as vscode.MarkdownString) || new vscode.MarkdownString('');
