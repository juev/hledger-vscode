...
const DEFAULT_PATTERNS: HeaderPatterns = {
  ...
  currency: [/^currency$/i, /^curr$/i, /^ccy$/i, /^валюта$/i],
};
...
export class ColumnDetector {
   ...
   */
  detectColumns(headers: readonly string[], sampleRows: readonly ParsedRow[]): ColumnMapping[] {
   ...
   */
  private analyzeColumnContent(values: readonly string[]): {
    type: ColumnType;
    confidence: number;
  } {
    if (values.length === 0) {
   ...
   */
  private mergePatterns(defaults: HeaderPatterns, custom: Partial<HeaderPatterns>): HeaderPatterns {
    ...
    for (const mapping of mappings) {
      ...
      if (!existing || mapping.confidence > existing.confidence) {
        ...
        if (existing) {
          ...
          const existingIndex = resolved.findIndex((m) => m.index === existing.index);
          if (existingIndex >= 0) {
  ...
  static hasRequiredColumns(mappings: readonly ColumnMapping[]): {
    ...
    const needsAmount = !mappings.some((m) => m.type === 'debit' || m.type === 'credit');

    if (needsAmount) {
      required.push('amount');
    }

    const missing = required.filter((type) => !mappings.some((m) => m.type === type));
