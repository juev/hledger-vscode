...
export class HLedgerLexer {
   ...
   */
  public tokenizeLine(line: string, _lineNumber: number): HLedgerToken {
  ...
  public parsePostingInfo(line: string): PostingToken {
    ...
    // Account name can contain single spaces.
    // The regex uses non-greedy matching (.*?) to capture up to the first
    // occurrence of: 2+ spaces, a tab character, semicolon/hash comment marker, or EOL.
    // This correctly handles all hledger format cases:
    //   - Account only: "Assets:Cash" → captures "Assets:Cash"
    //   - With amount: "Assets:Bank Account     200" → captures "Assets:Bank Account"
    //   - With comment: "Assets:Cash ; comment" → captures "Assets:Cash " (trimmed below)
    //   - With tab: "Assets:Cash\t100" → captures "Assets:Cash"
    const accountMatch = trimmed.match(/^(.*?)(?:\s{2,}|\t|[;#]|$)/);
    const accountName = (accountMatch?.[1] ?? trimmed).trim();
