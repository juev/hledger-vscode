...
export class HLedgerFileProcessor {
  ...
  public findHLedgerFiles(
    ...
    if (currentDepth >= maxDepth) {
      ...
      if (!process.env.JEST_WORKER_ID) {
        console.warn(
          `HLedger: Maximum depth (${maxDepth}) reached at ${dirPath}, stopping traversal`
        );
      }
      return [];
    }
    ...
    try {
      ...
      if (visitedDirs.has(realPath)) {
        ...
        if (!process.env.JEST_WORKER_ID) {
          console.warn(
            `HLedger: Symlink cycle detected at ${dirPath} (real: ${realPath}), skipping`
          );
        }
        return [];
      }
  ...
  private createMutableData(data: ParsedHLedgerData): any {
    ...
    return {
      ...
      payeeRecentTemplates: new Map(
        Array.from(data.payeeRecentTemplates.entries()).map(([k, v]) => [
          k,
          { keys: [...v.keys], writeIndex: v.writeIndex },
        ])
      ),
  ...
  public async detectFileType(
    ...
    try {
      ...
      const directiveLines = nonEmptyLines.filter((line) => {
        ...
        return /^(account|commodity|payee|tag|alias|include|format|decimal-mark|default)/i.test(
          trimmed
        );
      }).length;

      const postingLines = nonEmptyLines.filter(
        (line) =>
          line.length > 0 && (line[0] === ' ' || line[0] === '\t') && !line.trim().startsWith(';')
      ).length;
