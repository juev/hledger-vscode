...
describe('HLedgerFileProcessor Security Tests', () => {
  ...
  describe('path traversal protection', () => {
    it('should reject include paths that escape workspace with ..', async () => {
      ...
      expect(
        result.warnings.some(
          (w) => w.message.includes('security') || w.message.includes('outside workspace')
        )
      ).toBe(true);
    ...
    it('should reject absolute paths to system directories', async () => {
      ...
      expect(
        result.warnings.some(
          (w) => w.message.includes('security') || w.message.includes('system directory')
        )
      ).toBe(true);
    });
    ...
    it('should accept valid absolute paths within workspace', async () => {
      ...
      // Should not have security warnings
      expect(result.warnings.some((w) => w.message.includes('security'))).toBe(false);
