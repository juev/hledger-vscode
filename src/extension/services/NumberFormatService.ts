...
export class NumberFormatService {
  ...
  parseAmount(input: string): Result<ParsedAmount> {
    ...
    return failure(
      new Error(`Unable to parse amount: "${input}" does not match any known number format`)
    );
  }
  ...
  parseFormatTemplate(template: string): Result<CommodityFormat> {
    ...
    const templatePattern =
      /^(?:([\p{Sc}\p{Lu}]+)\s*)?(\p{N}[^\p{Lu}\p{Sc}]*\p{N}|\p{N}+)(?:\s*([\p{Sc}\p{Lu}]+))?$/u;
    ...
    const symbolSpacing = symbolBefore
      ...
      : /\s/.test(
          template.substring(
            template.indexOf(numberPart) + numberPart.length,
            template.indexOf(symbol)
          )
        );
  ...
  private tryParseWithFormat(input: string, format: NumberFormat): Result<ParsedAmount> {
    ...
    try {
      ...
      return failure(
        new Error(`Parse error: ${error instanceof Error ? error.message : 'Unknown error'}`)
      );
    }
  }
