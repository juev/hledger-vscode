...
export class StrictPositionAnalyzer {
  ...
  private isAfterDateWithSpace(beforeCursor: string): boolean {
    ...
    return (
      StrictPositionAnalyzer.STRICT_PATTERNS.AFTER_DATE_PATTERN.test(beforeCursor) ||
  ...
  private isInBalanceAssertionContext(beforeCursor: string): boolean {
    ...
    const balanceAssertionPattern =
      /^\s+[(\[]?[\p{L}][\p{L}\p{N}:_\s-]*[)\]]?(\s{2,}|\t)={1,2}\*?/u;
  ...
  private isAfterBalanceAssertionAmount(beforeCursor: string): boolean {
    ...
    const pattern =
      /^\s+[(\[]?[\p{L}][\p{L}\p{N}:_\s-]*[)\]]?(\s{2,}|\t)={1,2}\*?\s*[+-]?[\p{Sc}]?[+-]?\d+(?:[.,]\d*)?(?:[Ee][+-]?\d+)?\s$/u;
  ...
  private isDateContext(lineText: string, cursorPos: number): boolean {
    ...
    return (
      StrictPositionAnalyzer.STRICT_PATTERNS.DATE_LINE_START.test(beforeCursor) ||
